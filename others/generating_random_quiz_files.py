...
path = Path.cwd() / "automate_boring_stuff" / "files" / "35_quizzes"
path.mkdir(exist_ok=True, parents=True)

print(f"Directory exists: {path.exists()}")
print(f"Parent directories created: {path.parent.exists()}")
...
for quiz_num in range(35):
    try:
        ...
        quiz_file = open(path/f'capitalsquiz{quiz_num + 1}.txt', 'w')
        answer_key_file = open(path/f'capitalsquiz_answers{quiz_num + 1}.txt', 'w')
        ...
        for question_num in range(50):
            ...
            # Randomizing answers so that the correct response isn’t always choice D
        ...
        print(f"Quiz file created: {quiz_file.name}")
        print(f"Answer key file created: {answer_key_file.name}")
        quiz_file.close()
